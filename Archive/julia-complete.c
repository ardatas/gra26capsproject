...
void julia_k_iteration(float complex c, float complex start, size_t width, ssize_t height, float res, unsigned n,
        ...
        for (; x + 3 < width; x += 4) {
            ...
            if (check_interval == 1) {
                for (unsigned iteration = 0; iteration < n; ++iteration) {
                    ...
                    // 0x000000 0xffffff 0xffffff 0x000000 // 0x000000 0x000000 0x000000 0x000000 0x000000000000000000000000
...
void julia_count_optimization(float complex c, float complex start, size_t width, ssize_t height, float res, unsigned n,
                              bool color, unsigned char *img) {
#if !defined(__SSE2__)
    julia_V1(c, start, width, height, res, n, color, img);
#else
...
void julia_simd(float complex c, float complex start, size_t width, ssize_t height, float res, unsigned n, bool color,
                unsigned char *img) {
#if !defined(__SSE2__)
    julia_V1(c, start, width, height, res, n, color, img);
#else
    ...
    for (size_t y = 0; y < height_abs; ++y) {
        ...
        for (; x + 3 < width; x += 4) {
            ...
            for (unsigned iteration = 0; iteration < n; ++iteration) {
                ...
                // 0b0 0b1 0b1 0b0 -> 0x000000 0x000001 0x000001 0x000000
                // counts : // 0x000001 0x000002 0x000002 0x000000
