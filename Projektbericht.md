...
To reduce the overhead of checking `_mm_movemask_ps` every iteration, a check-interval parameter `K` was introduced: instead of testing the active mask after every step, it is tested only every `K` iterations, reducing the number of mask checks at the cost of running a few extra iterations on lanes that already escaped.
